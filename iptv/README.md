...
这是 [iptv-org/iptv](https://github.com/iptv-org/iptv) 中文频道播放列表 `languages/zho.m3u` 的名称派生版本，包含中国大陆、港澳台及海外中文频道。有可靠中文名的频道会显示中文；上游新增且暂时没有可靠中文名的频道会保留上游原频道名，避免错误翻译。频道分组、清晰度和状态提示会中文化；所有 CCTV 频道统一归入 `CCTV` 分组。频道标识、台标和直播地址保持上游原值。
...
GitHub Actions 每天北京时间约 06:20 拉取上游，重新生成三天中文节目单和频道订阅。只有内容发生变化时才提交。若上游新增频道但没有可靠中文名，会保留上游原频道名继续发布；若中文节目数量异常，或出现无法翻译的新分组和状态，更新会安全失败并保留上一版，避免发布空节目单或结构异常的订阅。
